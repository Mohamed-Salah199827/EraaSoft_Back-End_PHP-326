...
if (typeof BX.UI.EntityUserFieldManager === "undefined") {
  ...
  BX.UI.EntityUserFieldManager.prototype = {
    initialize: function (id, settings) {
      ...
      this._fieldEntityId = BX.prop.getString(
        this._settings,
        "fieldEntityId",
        ""
      );
      this._enableSelection = BX.prop.getBoolean(
        this._settings,
        "enableSelection",
        true
      );
      this._enableCreation = BX.prop.getBoolean(
        this._settings,
        "enableCreation",
        false
      );
      this._creationSignature = BX.prop.getString(
        this._settings,
        "creationSignature",
        ""
      );
      this._creationPageUrl = BX.prop.getString(
        this._settings,
        "creationPageUrl",
        ""
      );
      this._enableMandatoryControl = BX.prop.getBoolean(
        this._settings,
        "enableMandatoryControl",
        true
      );
      this._contextParams = BX.prop.getObject(
        this._settings,
        "contextParams",
        {}
      );
    ...
    getTypeInfos: function () {
      ...
      items.push({
        name: "address",
        title: BX.message("UI_ENTITY_EDITOR_UF_ADDRESS_TITLE_2"),
        legend: BX.message("UI_ENTITY_EDITOR_UF_ADDRESS_LEGEND_2")
      });

      items.push({
        name: "url",
        title: BX.message("UI_ENTITY_EDITOR_UF_URL_TITLE"),
        legend: BX.message("UI_ENTITY_EDITOR_UF_URL_LEGEND")
      });
      items.push({
        name: "file",
        title: BX.message("UI_ENTITY_EDITOR_UF_FILE_TITLE"),
        legend: BX.message("UI_ENTITY_EDITOR_UF_FILE_LEGEND")
      });
      items.push({
        name: "money",
    ...
    createField: function (fieldData, mode) {
      ...
      if (typeId === BX.UI.EntityUserFieldType.file) {
        ...
        if (
          typeId === BX.UI.EntityUserFieldType.employee ||
      ...
      BX.onCustomEvent(window, "BX.UI.EntityEditor:onUserFieldAdd", [
        this,
        fieldData
      ]);
    ...
    updateField: function (fieldData, mode) {
      ...
      BX.onCustomEvent(window, "BX.UI.EntityEditor:onBeforeUserFieldUpdate", [
        this,
        {
          fields: [fieldData],
          onSuccess: onSuccess
    ...
    createSchemeElement: function (fieldInfo) {
      return BX.UI.EntitySchemeElement.create(
        this.prepareSchemeElementSettings(fieldInfo)
      );
    },
    ...
    registerActiveField: function (field) {
      ...
      BX.Main.UF.EditManager.registerField(
        name,
        field.getFieldInfo(),
        field.getFieldNode()
      );
    ...
    onValidationComplete: function (results) {
      ...
      for (name in results) {
        ...
        if (this._activeFields.hasOwnProperty(name)) {
          ...
          this._validationResult.addError(
            BX.UI.EntityValidationError.create({ field: field })
          );
        }
      }
...
if (typeof BX.UI.EntityUserFieldLayoutLoader === "undefined") {
  ...
  BX.UI.EntityUserFieldLayoutLoader.prototype = {
    initialize: function (id, settings) {
      this._id = BX.type.isNotEmptyString(id) ? id : BX.util.getRandomString(4);
      this._settings = settings ? settings : {};
      this._mode = BX.prop.getInteger(
        this._settings,
        "mode",
        BX.UI.EntityEditorMode.view
      );
      this._enableBatchMode = BX.prop.getBoolean(
        this._settings,
        "enableBatchMode",
        true
      );
    ...
    startRequest: function () {
      ...
      if (this._mode === BX.UI.EntityEditorMode.view) {
        BX.Main.UF.Manager.getView(
          data,
          BX.delegate(this.onRequestComplete, this)
        );
      } else {
        BX.Main.UF.Manager.getEdit(
          data,
          BX.delegate(this.onRequestComplete, this)
        );
      }
...
if (typeof BX.UI.EntityEditorUserField === "undefined") {
  ...
  BX.UI.EntityEditorUserField.prototype.isTitleEnabled = function () {
    ...
    return (
      BX.prop.getString(
        BX.prop.getObject(info, "SETTINGS", {}),
        "DISPLAY",
        ""
      ) !== "CHECKBOX"
    );
  ...
  BX.UI.EntityEditorUserField.prototype.checkIfNotEmpty = function (value) {
    ...
    return BX.type.isArray(fieldValue)
      ? fieldValue.length > 0
      : fieldValue !== "";
  };
  ...
  BX.UI.EntityEditorUserField.prototype.layout = function (options) {
    ...
    if (fieldType === BX.UI.EntityUserFieldType.string) {
      BX.addClass(
        this._wrapper,
        "ui-entity-editor-content-block-field-custom-text"
      );
    } else if (
      fieldType === BX.UI.EntityUserFieldType.integer ||
      fieldType === BX.UI.EntityUserFieldType.double
    ) {
      BX.addClass(
        this._wrapper,
        "ui-entity-editor-content-block-field-custom-number"
      );
    } else if (fieldType === BX.UI.EntityUserFieldType.money) {
      BX.addClass(
        this._wrapper,
        "ui-entity-editor-content-block-field-custom-money"
      );
    } else if (
      fieldType === BX.UI.EntityUserFieldType.date ||
      fieldType === BX.UI.EntityUserFieldType.datetime
    ) {
      BX.addClass(
        this._wrapper,
        "ui-entity-editor-content-block-field-custom-date"
      );
      ...
      BX.addClass(
        this._wrapper,
        "ui-entity-editor-content-block-field-custom-file"
      );
    } else if (fieldType === BX.UI.EntityUserFieldType.url) {
      BX.addClass(
        this._wrapper,
        "ui-entity-editor-content-block-field-custom-link"
      );
    }
    ...
    if (this.hasContentToDisplay()) {
      ...
      if (html !== "") {
        ...
        if (!loader) {
          loader = BX.UI.EntityUserFieldLayoutLoader.create(this._id, {
            mode: this._mode,
        ...
        if (
          fieldType === BX.UI.EntityUserFieldType.file &&
          BX.type.isObject(fieldParams["ADDITIONAL"])
        ) {
          ...
          if (ownerToken !== "") {
            fieldParams["ADDITIONAL"]["URL_TEMPLATE"] +=
  ...
  BX.UI.EntityEditorUserField.prototype.adjustFieldParams = function (
    fieldParams,
    isLayoutContext
  ) {
    var fieldType = this.getFieldType();
  ...
  BX.UI.EntityEditorUserField.prototype.onLayoutSuccess = function () {
    ...
    if (fieldType === BX.UI.EntityUserFieldType.employee) {
      var button = this._innerWrapper.querySelector(
      ...
      if (button && this.isClickBinded) {
        BX.bind(
          button,
          "click",
          BX.delegate(this.onEmployeeSelectorOpen, this)
        );
    ...
    if (fieldType === BX.UI.EntityUserFieldType.boolean) {
      if (
        this._mode === BX.UI.EntityEditorMode.edit &&
        !this.checkIfNotEmpty(this.getValue())
      ) {
        this.markAsChanged();
      }
    }
    ...
    //KG-Edit-New
    //if(this.getName()=='UF_CRM_1688882089903'){
    if (this.getName() == "UF_CRM_1690361881405") {
      this.markAsChanged();
    }
  ...
  BX.UI.EntityEditorUserField.prototype.addExternalEventsHandlers =
    function () {
      this.removeExternalEventsHandlers();
      // Handler could be called by UF to trigger _changeHandler in complicated cases
      BX.addCustomEvent(
        window,
        "onUIEntityEditorUserFieldExternalChanged",
        BX.proxy(this.userFieldExternalChangedHandler, this)
      );
      BX.addCustomEvent(
        window,
        "onUIEntityEditorUserFieldSetValidator",
        BX.proxy(this.userFieldSetValidatorHandler, this)
      );
    };
  BX.UI.EntityEditorUserField.prototype.removeExternalEventsHandlers =
    function () {
  ...
  BX.UI.EntityEditorUserField.prototype.userFieldExternalChangedHandler =
    function (fieldId) {
  ...
  BX.UI.EntityEditorUserField.prototype.userFieldSetValidatorHandler =
  ...
  BX.UI.EntityEditorUserField.prototype.onLayoutLoaded = function (result) {
    ...
    if (html !== "") {
      // KG-Edit4
      // disable "Proposal document" upload field if Proposal approval status is approved or in progress
      if (html.includes("UF_CRM_1679346602")) {
        html = html.replace(
          'name="UF_CRM_1679346602"',
          'name="UF_CRM_1679346602" disabled'
        );
        const parser = new DOMParser();
        const doc = parser.parseFromString(html, "text/html");
        const selectField = doc.querySelector(
          'select[name="UF_CRM_1679346602"]'
        );
        const selectedValue = selectField.value;
        if (selectedValue == 406 || selectedValue == 407) {
          const disabledFields = document.querySelectorAll(".disabled_class");
          for (let i = 0; i < disabledFields.length; i++) {
            disabledFields[i].disabled = true;
          }
        }
      }
      // project id in ourchase request
      if (html.includes("UF_CRM_3_1685435445816")) {
        const urlParams = new URLSearchParams(window.location.search);
        const projectId = urlParams.get("project_id");
        if (projectId && projectId.trim() !== "") {
          html = html.replace(
            'value="' + projectId + '"',
            'value="' + projectId + '" selected'
          );
        }
      }
      ...
      //KG-Edit-New
      //ckeditor
      if (html.includes("UF_CRM_1690361881405")) {
        var htmlString = html;
        ...
        var inputElement = tempDiv.querySelector(
          'input[name="UF_CRM_1690361881405"]'
        );
        if (inputElement) {
          // Create a new textarea element
          var textareaElement = document.createElement("textarea");
          textareaElement.name = "UF_CRM_1690361881405";
          textareaElement.value = inputElement.value;
          textareaElement.innerHTML = inputElement.value;

          // Replace the input element with the textarea element
          var parentElement = inputElement.parentNode;
          parentElement.replaceChild(textareaElement, inputElement);
...
if (typeof BX.UI.EntityEditorUserFieldListItem === "undefined") {
  ...
  BX.extend(
    ...
    BX.UI.EntityEditorFieldConfiguratorEnumItem
  );
...
if (typeof BX.UI.UserFieldTypeMenu === "undefined") {
  BX.UI.UserFieldTypeMenu = function () {
    ...
    this._bottomButtonMouseOverHandler = BX.delegate(
      this.onBottomButtonMouseOver,
      this
    );
    this._bottomButtonMouseOutHandler = BX.delegate(
      this.onBottomButtonMouseOut,
      this
    );

    this._topButtonMouseOverHandler = BX.delegate(
      this.onTopButtonMouseOver,
      this
    );
    this._topButtonMouseOutHandler = BX.delegate(
      this.onTopButtonMouseOut,
      this
    );
  ...
  BX.UI.UserFieldTypeMenu.prototype = {
    ...
    open: function (anchor) {
      ...
      this._popup = new BX.PopupWindow(this._id, anchor, {
        autoHide: true,
        draggable: false,
        offsetLeft: 0,
        offsetTop: 0,
    ...
    prepareContent: function () {
      this._wrapper = BX.create("div", {
        props: { className: "ui-entity-editor-popup-create-field-popup" }
      });
      ...
      this._topScrollButton = BX.create("div", {
        props: {
          className: "ui-entity-editor-popup-create-scroll-control-top"
        },
        html: scrollIcon
      });
      this._wrapper.appendChild(this._topScrollButton);

      this._bottomScrollButton = BX.create("div", {
        props: {
          className: "ui-entity-editor-popup-create-scroll-control-bottom"
        },
        html: scrollIcon
      });
      ...
      this._innerWrapper = BX.create("div", {
        props: { className: "ui-entity-editor-popup-create-field-list" }
      });
      this._wrapper.appendChild(this._innerWrapper);
    ...
    onPopupShow: function () {
      ...
      BX.bind(
        this._bottomScrollButton,
        "mouseover",
        this._bottomButtonMouseOverHandler
      );
      BX.bind(
        this._bottomScrollButton,
        "mouseout",
        this._bottomButtonMouseOutHandler
      );

      BX.bind(
        this._topScrollButton,
        "mouseover",
        this._topButtonMouseOverHandler
      );
      BX.bind(
        this._topScrollButton,
        "mouseout",
        this._topButtonMouseOutHandler
      );
  ...
  BX.UI.UserFieldTypeMenuItem.prototype = {
    ...
    prepareContent: function () {
      ...
      wrapper.appendChild(
        BX.create("span", {
          props: {
            className: "ui-entity-editor-popup-create-field-item-title"
          },
          text: this._text
        })
      );
...
if (typeof BX.UI.EntityEditorUserFieldConfigurator === "undefined") {
  ...
  BX.extend(
    BX.UI.EntityEditorUserFieldConfigurator,
  ...
  BX.UI.EntityEditorUserFieldConfigurator.prototype.getDefaultFieldLabel =
    function () {
      var manager = this._editor.getUserFieldManager();
      return manager.getDefaultFieldLabel(this._typeId);
    };

  BX.UI.EntityEditorUserFieldConfigurator.prototype.getInputTitle =
    function () {
      var manager = this._editor.getUserFieldManager();
      return this._field
        ? this._field.getTitle()
        : manager.getDefaultFieldLabel(this._typeId);
    };
  BX.UI.EntityEditorUserFieldConfigurator.prototype.getInputContainer =
    function () {
  ...
  BX.UI.EntityEditorUserFieldConfigurator.prototype.layoutInnerConfigurator =
    function (innerConfig, listItems, nextNode) {
      if (
        ...
        var fieldSettings =
          this._settings.field._schemeElement._data.fieldInfo.SETTINGS;
  ...
  BX.UI.EntityEditorUserFieldConfigurator.prototype.layoutInternal =
    function () {
      this._wrapper.appendChild(this.getInputContainer());

      if (
        this._typeId === BX.UI.EntityUserFieldType.enumeration &&
        (!this._field || this._field.getEditor().canChangeCommonConfiguration())
      ) {
        if (this._enumConfigurator === null) {
          ...
          this._enumConfigurator =
            BX.UI.EntityEditorUserFieldEnumConfigurator.create({
  ...
  BX.UI.EntityEditorUserFieldConfigurator.prototype.getOptionContainer =
    function () {
      var isNew = this._field === null;
      this._optionWrapper = BX.create("div", {
        props: { className: "ui-entity-editor-content-block" }
      });
      this._wrapper.appendChild(
        BX.create("div", {
          props: {
            className:
              "ui-entity-editor-content-block ui-entity-editor-content-block-checkbox"
          },
      ...
      if (isNew && (this._typeId === "datetime" || this._typeId === "date")) {
        this._isTimeEnabledCheckBox = this.createOption({
      ...
      if (this._typeId !== "boolean") {
        if (
          ...
          if (this._mandatoryConfigurator) {
            this._isRequiredCheckBox = this.createOption({
              ...
              containerSettings: {
                props: { className: "ui-entity-new-field-addiction-flex-row" }
              },
              elements: this._mandatoryConfigurator.getButton().prepareLayout()
            });

            this._isRequiredCheckBox.checked =
              (this._field && this._field.isRequired()) ||
              this._mandatoryConfigurator.isCustomized();

            this._mandatoryConfigurator.setSwitchCheckBox(
              this._isRequiredCheckBox
            );
            this._mandatoryConfigurator.setLabel(
              this._isRequiredCheckBox.nextSibling
            );

            this._mandatoryConfigurator.setEnabled(
              this._isRequiredCheckBox.checked
            );
        ...
        if (isNew) {
          this._isMultipleCheckBox = this.createOption({
      ...
      this._showAlwaysCheckBox.checked = isNew
        ...
        : this._field.checkOptionFlag(
            BX.UI.EntityEditorControlOptions.showAlways
          );
      //endregion
  ...
  BX.UI.EntityEditorUserFieldConfigurator.prototype.prepareSaveParams =
    function (e) {
      var params =
        BX.UI.EntityEditorUserFieldConfigurator.superclass.prepareSaveParams.apply(
          this,
          arguments
        );
      if (
  ...
  BX.UI.EntityEditorUserFieldConfigurator.prototype.getIsRequiredCheckBox =
    function () {
      var checkBox = null;
      if (this._typeId !== "boolean") {
...
if (typeof BX.UI.EntityEditorUserFieldEnumConfigurator === "undefined") {
  BX.UI.EntityEditorUserFieldEnumConfigurator = function () {
    BX.UI.EntityEditorUserFieldEnumConfigurator.superclass.constructor.apply(
      this
    );
  };
  BX.extend(
    BX.UI.EntityEditorUserFieldEnumConfigurator,
    BX.UI.EntityEditorEnumConfigurator
  );

  BX.UI.EntityEditorUserFieldEnumConfigurator.prototype.createEnumerationItem =
    function (data) {
